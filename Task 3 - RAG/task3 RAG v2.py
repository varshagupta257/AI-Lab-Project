...
# Using the cleaner 1000/250 chunk setting
...
# Switched from Chroma to FAISS vector store
vector_store = FAISS.from_documents(policy_chunks, embeddings)
# Set search kwargs to k=7 chunks
...
# ==========================================
llm = ChatOllama(
    model="mistral", # Switched to Mistral as requested
    temperature=0.7, # Adjusted temperature to 0.7
    format="json",
    timeout=30.0  
)
...
print(f"\n🚀 Starting streaming batch processing using Mistral via FAISS (k=7) at Temp 0.7...")
...
for index, row in df.iterrows():
    ...
    # Prompt left optimized without micro-managed hardcoding
    prompt_text = f"""You are a strict financial auditor. Match the transaction data to the provided corporate policy context. 
Analyze the rules and exceptions dynamically from the context below to find discrepancies between employee levels, categories, caps, or prohibited practices.
...
"""
    ...
    print(f"⏳ [{current_row_num}/{total_rows}] Saved TXN: {row['Transaction_ID']} | Category: {rag_cat} | Violation: {rag_viol} | Took: {row_elapsed:.2f}s")
